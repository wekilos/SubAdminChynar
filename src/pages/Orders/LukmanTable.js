...
const LukmanTable = (props) => {
      ...
      ? [
...

          {
            ...
            render: (text, record) => (
              <div>
                ...
                >
                  Dastawka:
                  {record?.Market?.MarketDeleiveries?.map((item) => {
                    return (
                      item.id == record?.delivery_time_status && item.price
                    );
                  })}
                </h1>
                <h1
                ...
                >
                  ...
                  {record?.Market?.MarketDeleiveries?.map((item) => {
                    return (
                      item.id == record?.delivery_time_status &&
                      item.description_tm
                    );
                  })}
                </h1>
          ...
          },
          {
            ...
            render: (text, record) => (
              <div>
                <p>
                  {" "}
                  {record.order_date_time &&
                    record.order_date_time.slice(0, 10)}
                </p>
                <p>
                  {record.order_date_time &&
                    record.order_date_time.slice(11, 19)}
                </p>
              </div>
            ),
          },
          {
            ...
            render: (text, record) => (
              <div>
                {record.is_cash &&
                  (dil === "TM" ? "Nagt töleg" : "Наличный расчет")}
                {!record.is_cash &&
                  (dil === "TM" ? "Kart bilen töleg" : "Оплата картой")}
              </div>
            ),
          },

          {
            ...
            render: (text, record) => (
              <div>
                {record.Status && dil === "TM"
                  ? record.Status.name_tm
                  : record.Status.name_ru}
              </div>
            ),
          },
          {
            ...
            render: (text, record) => (
              <div>
                ...
                <p>
                  {record.Address && record.Address.rec_address}{" "}
                  {record.Address && record.Address.rec_number}
                </p>
              </div>
            ),
          },
          {
            ...
            render: (text, record) => (
              <div>
                <h3>
                  {record && record.User && record.User.fname}{" "}
                  {record && record.User && record.User.lastname}{" "}
                </h3>
          ...
          },
          {
            ...
            render: (text, record) => (
              <div>
                <Button
                  style={{ marginBottom: "5px", width: "145px" }}
                  type="primary"
                  shape="round"
                ...
                <Popconfirm
                  title={
                    dil === "TM" ? "Haryt Gowşuryldymy?" : "Товар отправлен?"
                  }
                ...
                >
                  <Button
                    ...
                    shape="round"
                  >
                    {dil === "TM" ? "Gowşuryldy" : "Доставленный"}
                  </Button>
                </Popconfirm>
                <br></br>
                <Button
                ...
                <Popconfirm
                  title={
                    dil === "TM"
                      ? "Siz çyndan öçürmek isleýärsinizmi?"
      ...
      : [
          ...
          },
          {
            ...
            render: (text, record) => (
              <div>
                <Button
                  style={{ marginBottom: "5px", width: "105px" }}
                  type="primary"
                  shape="round"
                  onClick={() => ShowInformation(record)}
                >
  ...
  const DeleteOrder = (event) => {
    console.log(event);
    axiosInstance
      .delete("/api/order/delete/" + event.id)
      .then((data) => {
  ...
  const Gowshuryldy = (event) => {
    axiosInstance
      .patch("/api/order/deliveri/" + event.id)
      .then((data) => {
        message.success(data.data.msg);
        getOrders();
      })
      ...
      >
        {maglumat && (
          <table style={{ width: "100%" }} border="1" className="goshmacha--ul">
            ...
            {maglumat &&
              maglumat.OrderedProducts &&
              maglumat.OrderedProducts.map((product, i) => {
                return (
                  <React.Fragment>
                    <tr className="modalLi" key={`toleg${i}`}>
                      <td>
                        {i + 1}) {dil === "TM" ? "Haryt No" : "Продукт нo"}
                      </td>
                    ...
                    <tr className="modalLi" key={`tolega${i}`}>
                      <td>
                        {i + 1}){" "}
                        {dil === "TM" ? "Haryt Ady" : "Название продукта"}
                      </td>
                      <td>
                        {dil === "TM"
                    ...
                    <tr className="modalLi" key={`tolegd${i}`}>
                      ...
                      <td>
                        {dil === "TM"
                          ? product.Product.description_tm
                          : product.Product.description_ru}{" "}
                      </td>
                    </tr>
                    <tr className="modalLi" key={`tolegpu${i}`}>
                      <td>
                        {i + 1}){" "}
                        {dil === "TM" ? "Haryt ölçegi" : "Тип продажи продукта"}
                      </td>
                      <td>
                        {dil === "TM"
                          ? product.Product.Unit.name_tm
                          : product.Product.Unit.name_ru}{" "}
                      </td>
                    </tr>
                    <tr className="modalLi" key={`tolegp${i}`}>
                      <td>
                        {i + 1}) {dil === "TM" ? "Haryt baha" : "Цена товара"}
                      </td>
                      <td>
                        {product.Product.is_valyuta_price == true
                          ? (product.Product.price * valyuta).toFixed(2)
                    ...
                    <tr className="modalLi" key={`tolegsp${i}`}>
                      ...
                      <td>
                        {product.Product.is_valyuta_price == true
                          ? (product.Product.sale_price * valyuta).toFixed(2)
                    ...
                    <tr className="modalLi" key={`sany${i}`}>
                      <td>
                        {i + 1}){" "}
                        {dil === "TM" ? "Haryt Sany" : "количество товаров"}
                      </td>
                      <td>{product.amount} </td>
                    </tr>
                    {product.razmer && (
                      <tr className="modalLi" key={`sanyr${i}`}>
                        <td>
                          {i + 1}){" "}
                          {dil === "TM" ? "Haryt Razmer" : "Размер товара"}
                        </td>
                        <td>{product.razmer} </td>
                      </tr>
                    )}
                    {product.renk && (
                      <tr className="modalLi" key={`sanyre${i}`}>
                        <td>
                          {i + 1}){" "}
                          {dil === "TM" ? "Haryt Renki" : "Цвет продукта"}
                        </td>
                        <td>{product.renk} </td>
                      </tr>
                    )}
                    ...
                    <tr className="modalLi" key={`suratM${i}`}>
                      <td>
                        ...
                        {product.Product &&
                          ...
                          (dil === "TM"
                            ? product.Product.Market.name_tm
                            : product.Product.Market.name_ru)}
                      </td>
                      <img
                        src={
                          BASE_URL +
