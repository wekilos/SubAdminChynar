...
const LukmanTable = props=>{
    ...
    const columns = [
        ...
        {
            title:"Footer telofon",
            dataIndex:"footer_phone_number"
        },
        {
            title:"Işe başlaýan wagty",
            dataIndex:"work_start_time",
            render:(text,record)=>(
                <div>
                    {record.work_start_time.slice(0,10)} {record.work_start_time.slice(11,18)}
                </div>
            )
        },
        {
            title:"Işiň gutarýan wagty",
            dataIndex:"work_start_time",
            render:(text,record)=>(
                <div>
                    {record.work_end_time.slice(0,10)} {record.work_end_time.slice(11,18)}
                </div>
            )
        },
        {
...
const saveData = (event)=>{
    ...
    return(
        <div className='LukmanTable'>
                ...
                onClose={()=>ShowDrawer()}
                visible={edit}>
                    <StatusEdit getConfig={getConfig} mag={[maglumat,setMaglumat]} onClick={ShowDrawer}  />
                </Drawer>
